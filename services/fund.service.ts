...
export class FundService {
    ...
    async getLatestState(chainId: string, userId: string) {
    ...
    async forceSyncState(chainId: string, state: any) {
        if (!state.balances) return

        const smt = this._buildSmt(state.balances)
        if (smt.getRoot() !== state.root) {
            throw new BadRequestException('Malicious peer detected: balances map does not match the verified root')
        }

        await this.fundModel.updateOne(
            { chainId },
            { $set: { balances: state.balances, root: state.root, nonce: state.nonce, signature: state.signature } },
            { upsert: true }
        )
    }
    ...
    async commitTransaction(payload: TxPayload, signature: Signature) {
        ...
        if (chain && chain.signature && chain.signature.s === signature.s) {
        ...
        await this.fundModel.updateOne({ chainId }, { $set: { balances, root, nonce, signature } }, { upsert: true })
    }
