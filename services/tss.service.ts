...
export class TssService {
    ...
    async proposeTransaction(chainId: string, userId: string, amountHex: string) {
        ...
        if (latestState.nonce > localState.nonce) {
            ...
            await this.fundService.forceSyncState(chainId, latestState)
        }
        ...
        this.receiveProposal(this.nodeId, messageHash, payload)
        const peersWithoutMe = this.nodes.filter((n) => n.id !== this.nodeId)
        peersWithoutMe.forEach((peer) => {
            this.httpService
                .post(`${peer.url}/fund/propose`, {
                    i: this.nodeId,
                    messageHash,
                    payload,
                })
    ...
    async receiveProposal(i: number, messageHash: string, payload: any): Promise<{ status: string }> {
        await this.proposalModel.updateMany(
            { 'payload.chainId': payload.chainId, proposers: i },
            { $pull: { proposers: i } }
        )
        ...
        const proposal = await this.proposalModel.findOneAndUpdate(
            { messageHash },
            { $set: { chainId: payload.chainId, payload }, $addToSet: { proposers: i } },
        ...
        if (proposal.proposers.length === this.threshold) {
            ...
            if (subsetIds.includes(this.nodeId)) {
                this._executeTssSession(messageHash, subsetIds, proposal.payload).catch(console.error)
            }
        }
        return { status: 'OK' }
    }

    private async _executeTssSession(messageHash: string, subsetIds: number[], payload: any) {
        const key = await this.keyModel.findOne({})
        ...
        const E_x_i = P.encrypt(key.paillier.publicKey, w_i).toString(16)

        const state = await this.tssStateModel.findOneAndUpdate(
            { messageHash },
            {
                $setOnInsert: { payload, subsetIds },
                $set: {
                    k_i: k_i.toString(16),
        ...
        const peers = this.nodes.filter((n) => subsetIds.includes(n.id) && n.id !== this.nodeId)
        peers.forEach((peer) => {
            this.httpService
        ...
        if ((state.starts?.length || 0) === this.threshold && state.k_i && !state.w_i) {
            await this.transitionToMtA(messageHash, state).catch(console.error)
        }
    }

    async tssStart(i: number, messageHash: string, E_k: string, E_x: string, Gamma: string): Promise<TssStartResponse> {
        const state = await this.tssStateModel.findOneAndUpdate(
            { messageHash },
            {
    ...
    private async transitionToMtA(messageHash: string, state: TssStateDocument) {
        ...
        for (const peerStart of state.starts) {
            ...
            const term1_x = P.multiply(otherPublicKey, BN.from(peerStart.E_x, 16), BN.from(state.k_i, 16))
        ...
        if ((updatedState.mtas?.length || 0) === this.threshold - 1 && !updatedState.sigma_i) {
            await this.transitionToDelta(messageHash, updatedState)
        }
    }

    async tssMta(messageHash: string, from: number, alpha: string, nu: string): Promise<TssMtaResponse> {
        const state = await this.tssStateModel.findOneAndUpdate(
            { messageHash },
        ...
        if (
            ...
            await this.transitionToDelta(messageHash, state).catch(console.error)
        }
        return { status: 'OK' }
    }
    ...
    async tssDelta(messageHash: string, from: number, delta: string): Promise<TssDeltaResponse> {
        const state = await this.tssStateModel.findOneAndUpdate(
            { messageHash },
        ...
        if (
            ...
            await this.transitionToSign(messageHash, state).catch(console.error)
        }
        return { status: 'OK' }
    }
    ...
    async tssSign(messageHash: string, from: number, s_i: string): Promise<TssSignResponse> {
        ...
        if ((state.signs?.length || 0) === this.threshold && state.r && state.r !== 'processing') {
            state = await this.tssStateModel.findOneAndUpdate(
