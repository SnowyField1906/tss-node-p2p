...
const createMock = (id: number) => {
    ...
    const proposalModel = new MockModel() as any
...
describe('Threshold Signature Scheme P2P (n=3, t=2)', () => {
    ...
    const routePhaseRequests = async (endpoint: string) => {
        ...
        while (networkQueue.length > 0) {
            ...
            if (req.url.includes(endpoint)) {
                ...
                if (destNode) {
                    routed = true
                    if (req.url.includes('/tss/start'))
                        await destNode.tssService.tssStart(
                            req.data.i,
                            req.data.messageHash,
                            req.data.E_k,
                            req.data.E_x,
                            req.data.Gamma
                        )
                    else if (req.url.includes('/tss/mta'))
                        await destNode.tssService.tssMta(
                            req.data.messageHash,
                            req.data.from,
                            req.data.alpha,
                            req.data.nu
                        )
                    else if (req.url.includes('/tss/delta'))
                        await destNode.tssService.tssDelta(req.data.messageHash, req.data.from, req.data.delta)
                    else if (req.url.includes('/tss/sign'))
                        await destNode.tssService.tssSign(req.data.messageHash, req.data.from, req.data.s_i)
                    else if (req.url.includes('/dkg/broadcast')) {
                    ...
                    } else if (req.url.includes('/fund/propose')) {
                        await destNode.tssService.receiveProposal(req.data.i, req.data.messageHash, req.data.payload)
                    }
                }
            } else {
    ...
    it('Phase 0 & 1: Nodes propose transaction and process Start Phase', async () => {
        ...
        await routePhaseRequests('/fund/propose')
