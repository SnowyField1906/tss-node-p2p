...
- Dispatches the proposal directly to all peers via `POST /fund/propose`. The node keeps distinct `pendingTransaction` records separated by `chainId` to prevent overlapping states.
...
```bash
yarn dev:node1 # tss-node-p2p
yarn dev:node2 # tss-node-p2p
yarn dev:node3 # tss-node-p2p
```
